    ...
    <div className="fixed inset-0 bg-navy-principal/80 z-50 flex items-center justify-center p-4">
      <div className="bg-white-pure rounded-3xl w-full max-w-lg p-6 space-y-6">
        <div className="flex items-center justify-between">
          <h2 className="text-xl font-serif font-bold text-navy-principal">{t('cardManager.title')}</h2>
        ...
        <div className="space-y-4 max-h-[60vh] overflow-y-auto">
          {data.cartoes?.map((c) => (
            <div key={c.id} className="bg-navy-principal text-white-pure p-5 rounded-2xl flex items-center justify-between shadow-lg">
              <div>
                ...
                <p className="text-[10px] text-gray-light mt-2 uppercase tracking-widest">{t('cardManager.dates', c.diaFechamento.toString(), c.diaVencimento.toString())}</p>
              </div>
          ...
          {!isAdding && (
            <button onClick={() => setIsAdding(true)} className="w-full py-4 border-2 border-dashed border-gray-soft rounded-2xl flex items-center justify-center gap-2 text-gray-medium font-bold hover:border-gold-principal hover:text-gold-principal">
              <Plus size={20} /> {t('cardManager.add')}
            </button>
          )}
        </div>
        ...
        {isAdding && (
          <form onSubmit={handleSubmit} className="space-y-4 pt-4 border-t border-gray-soft">
            <input placeholder={t('cardManager.form.nickname')} className="w-full p-3 bg-white-off rounded-xl border border-gray-soft" onChange={e => setFormData({...formData, nome: e.target.value})} required />
            <input placeholder={t('cardManager.form.bank')} className="w-full p-3 bg-white-off rounded-xl border border-gray-soft" onChange={e => setFormData({...formData, banco: e.target.value})} required />
            ...
            <div className="grid grid-cols-2 gap-4">
              <input type="number" placeholder={t('cardManager.form.closingDay')} className="w-full p-3 bg-white-off rounded-xl border border-gray-soft" onChange={e => setFormData({...formData, diaFechamento: parseInt(e.target.value)})} min="1" max="28" required />
              <input type="number" placeholder={t('cardManager.form.dueDay')} className="w-full p-3 bg-white-off rounded-xl border border-gray-soft" onChange={e => setFormData({...formData, diaVencimento: parseInt(e.target.value)})} min="1" max="28" required />
            </div>
            <button className="w-full py-3 bg-navy-principal text-white-pure rounded-xl font-bold">{t('cardManager.form.save')}</button>
          </form>
        )}
      </div>
    </div>
